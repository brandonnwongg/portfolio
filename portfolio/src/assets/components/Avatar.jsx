...
export function Avatar(props) {
  ...
  const { animations: sleeping } = useFBX('/animations/sleep.fbx');
  const { animations: entering } = useFBX('/animations/entering.fbx');
  const { animations: standing } = useFBX('/animations/standing.fbx');
  const { animations: falling } = useFBX('/animations/falling.fbx');

  sleeping[0].name = 'Sleeping';
  entering[0].name = 'Entering';
  standing[0].name = 'Standing';
  falling[0].name = 'Falling';

  const { actions } =  useAnimations([sleeping[0], entering[0], standing[0], falling[0]], group)
  ...
  return (
    <group {...props} ref={group} dispose={null}>
      ...
      <skinnedMesh geometry={nodes.Wolf3D_Headwear.geometry} material={materials.Wolf3D_Headwear} skeleton={nodes.Wolf3D_Headwear.skeleton} frustumCulled={false}/>
